...
import cz.artique.jade.bookTrader.ontology.*;
...
public class BookTrader extends Agent {
    ...
    class BookEvaluator extends TickerBehaviour {

        private HashMap<String, Double> forSell;

        private HashMap<String, Double> forBuy;

        private HashSet<String> books;

        public BookEvaluator(Agent a, long period) {
            super(a, period);

            for (Goal g : myGoal) {
                forBuy.put(g.getBook().getBookName(), g.getValue());
                books.add(g.getBook().getBookName());
            }
        }

        public Double getForSell(String book) {
            books.add(book);
            return forSell.get(book);
        }
        
        public Double getForBuy(String book) {
            books.add(book);
            return forBuy.get(book);
        }
